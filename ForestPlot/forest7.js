...
//            .type(d3.symbolTriangle).size(50);
    ...
    .ticks(nTicks)
    .tickFormat(x =>{
            if (logr==="OD")  {return x.toFixed(1)}
            if (logr==="TD")  {return x.toFixed(2)}
            if (logr==="EXP")  {return `${Math.exp(x).toFixed(1)}`}
            })
...
function readfile(e){
    ...
    reader.addEventListener('load', e => {
        ...
        effL.push(rmin)
        num0=(+Math.min(...effL)-xoff)
        rminL=(Math.round((num0 + Number.EPSILON) * 100) / 100).toFixed(3)
        ...
        //console.log("first",data[0].refLinePos)
...
$('.pt').change(function() {
    ...
    console.log(value)
    if (value=="No"){ptr="No"; }
    else if (value=="Yes" && Dheaders.includes("pvPos")){ptr="Yes";}
     else if (value=="Yes" && !Dheaders.includes("pvPos"))
             {ptr="No";
             alert("please input pvPos in your dataset!")
             $(this).val('No')
             }
    $("#svg-forestplot").remove();
  svggraph()
...
$('.cs').change(function() {
    ...
    var value = $option.val();//to get content of "value" attrib
     ...
     if (value=="No"){csr="No"; }
     else if (value=="Yes" && Dheaders.includes("symbolsize") && Dheaders.includes("symboltype") && Dheaders.includes("symbolcolor"))
         {csr="Yes";}
     else if (value=="Yes" && (!Dheaders.includes("symbolsize") || !Dheaders.includes("symboltype") || !Dheaders.includes("symbolcolor")))
         {csr="No"
         alert("please input symbolsize, symboltype, symbolcolor in your dataset!")
         $(this).val('No')
         }
